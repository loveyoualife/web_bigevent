$(function() {
  ...
  // 通过代理的形式，为 btn-list 表单绑定点击事件
  var indexEdit = null
  $('tbody').on('click', '.btn-list', function() {
      // 弹出一个修改文章分类信息的层
      indexEdit = layer.open({
        type: 1,
        area: ['500px', '250px'],
        title: '修改文章分类',
        content: $('#dialog-list').html()
      })
      ...
      $.ajax({
          method: 'GET',
          url: 'http://big-event-api-t.itheima.net/my/article/list/' + id,
          headers: {
            Authorization: localStorage.getItem('token') || ''
          },
          success: function(res) {
              console.log(res)
            form.val('form-list', res.data)
          }
      })
  })
  ...
  $('body').on('submit', '#form-list', function(e) {
    e.preventDefault()
    $.ajax({
          method: 'POST',
